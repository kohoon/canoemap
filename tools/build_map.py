...
from weir_selection import select_weirs
weirs = []
...
<style>
  ...
  .leaflet-div-icon.river-wave-icon{background:transparent;border:0;pointer-events:none}
  .river-wave-mark{display:block;width:20px;height:12px;transform-origin:50% 50%;filter:drop-shadow(0 1px 1px rgba(10,111,149,.42))}
  ...
  .obs-ic.obs-icon-only{justify-content:center;width:26px;height:26px;box-sizing:border-box;padding:0;border-radius:50%;font-size:14px;line-height:1}
  ...
  .leaflet-control-layers-expanded{width:206px;color:#222}
  ...
  .lc-group-tag{display:none!important}
  .lc-section{margin-top:7px;padding-top:6px;border-top:1px solid #e5eaec}
  .lc-section:first-child{margin-top:0;padding-top:0;border-top:0}
  .lc-section-title{margin:0 0 3px;color:#718087;font:800 10px/1.25 sans-serif;letter-spacing:.02em}
  .lc-section-body>label{display:block}
  .lc-water-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:3px}
  .lc-water-row label{display:flex;align-items:center;min-width:0;margin:0;padding:4px 2px;border:1px solid #dbe4e8;border-radius:6px;background:#f7fafb;font-size:10.5px;white-space:nowrap}
  .lc-water-row label>span{display:flex;align-items:center;min-width:0}
  .lc-water-row .rv-sw{margin-right:1px}
  .lc-store-row{display:grid;grid-template-columns:1fr 1fr;gap:4px}
  .lc-store-row label{display:flex;align-items:center;margin:0;padding:4px;border:1px solid #e1e7e9;border-radius:6px;background:#fff}
  .lc-store-row label>span{display:flex;align-items:center}
  ...
  .sat-base-row{display:flex!important;align-items:center;gap:4px}
  .sat-base-row>span{display:flex;align-items:center;flex:1;min-width:0}
  .wayback-open{flex:none;border:1px solid #90a4ae;border-radius:7px;background:#f7fbff;color:#254b65;padding:5px 7px;font:700 11px sans-serif;white-space:nowrap;cursor:pointer;box-shadow:0 1px 2px rgba(0,0,0,.08)}
  ...
  .wayback-mode{display:flex;margin-top:6px;border:1px solid #afc2d4;border-radius:6px;overflow:hidden;background:#fff}
  .wayback-mode button{flex:1;border:0;border-left:1px solid #d3def0;background:#fff;color:#41566b;padding:6px 4px;font:700 11px/1.1 sans-serif;cursor:pointer}
  .wayback-mode button:first-child{border-left:0}
  .wayback-mode button.on{background:#185fa5;color:#fff}
  ...
  .wayback-now{left:25%}
  .wayback-old{left:75%}
  .wayback-compare:not(.split) .wayback-now,.wayback-compare:not(.split) .wayback-divider{display:none}
  .wayback-compare:not(.split) .wayback-old{left:50%}
  ...
  .locbtn.active{background:#1976d2;box-shadow:0 1px 4px rgba(0,0,0,.3),0 0 0 3px rgba(25,118,210,.2)}
  .locbtn.active svg{fill:#fff!important}
  ...
  .addr-drag-handle{display:flex;align-items:center;justify-content:center;height:18px;margin:-8px 26px 3px 0;cursor:grab;user-select:none;touch-action:none}
  .addr-drag-handle:active{cursor:grabbing}
  .addr-drag-grip{display:block;width:34px;height:4px;border-radius:999px;background:#c9d3d8;transition:width .12s ease,background .12s ease}
  .addr-drag-handle:hover .addr-drag-grip,.addr-drag-handle:active .addr-drag-grip{width:42px;background:#8ea1aa}
  .leaflet-popup.addr-popup{will-change:margin-left,margin-bottom}
  ...
  .loc-transport-in{display:flex;align-items:center;justify-content:center;width:46px;height:42px;filter:drop-shadow(0 2px 4px rgba(13,71,161,.65))}
  .loc-transport-in svg{width:44px;max-height:40px;height:auto;overflow:visible}
  .loc-transport-in.canoe{animation:locbob 2.4s ease-in-out infinite}
  .loc-transport-in.person svg{width:30px}
  ...
  .daiso-pin{display:flex;align-items:center;justify-content:center;width:34px;height:34px;filter:drop-shadow(0 2px 3px rgba(0,0,0,.48))}
  ...
  .offline-btn{cursor:pointer;font:800 12.5px/1.2 sans-serif;background:#0f6174;color:#fff;padding:9px 11px;border-radius:8px;box-shadow:0 2px 7px rgba(0,0,0,.3);white-space:nowrap;user-select:none;border:0}
  html.tour-mode #offlineCtl{position:fixed;left:10px;top:max(52px,calc(env(safe-area-inset-top,0px) + 48px))}
  .offline-banner{position:fixed;left:50%;top:max(9px,env(safe-area-inset-top,0px));transform:translateX(-50%);z-index:2100;display:none;align-items:center;gap:7px;max-width:calc(100vw - 180px);box-sizing:border-box;padding:7px 11px;border-radius:18px;background:#0f6174;color:#fff;font:800 12px/1.2 sans-serif;box-shadow:0 2px 9px rgba(0,0,0,.32);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer}
  .offline-banner.warn{background:#8b4b00}.offline-banner.on{display:flex}
  #offlineModal .pmodal{max-width:500px}
  .offline-status{border:1px solid #d5e2e5;background:#f2f8f8;border-radius:11px;padding:11px 12px;margin:10px 0;color:#345;font:13px/1.5 sans-serif}
  .offline-status b{display:block;color:#0f6174;font-size:14px}.offline-status small{display:block;color:#74858b;margin-top:2px}
  .offline-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0 14px}
  .offline-actions button{border:0;border-radius:10px;padding:11px 8px;font:800 13px sans-serif;cursor:pointer}.offline-save{background:#0f6174;color:#fff}.offline-use{background:#e3f0f2;color:#0b5666}.offline-delete{grid-column:1/-1;background:#f3eeee;color:#a23b3b}
  .offline-actions button:disabled{opacity:.48;cursor:default}
  .offline-progress{height:6px;border-radius:6px;background:#dfe8ea;overflow:hidden;margin:8px 0;display:none}.offline-progress.on{display:block}.offline-progress i{display:block;width:0;height:100%;background:#00a2b8;transition:width .2s}
  .offline-guide{margin:12px 0 0;padding:12px 12px 12px 29px;border-top:1px solid #e4eaec;color:#344b52;font:13px/1.55 sans-serif}.offline-guide li{margin:5px 0;padding-left:2px}.offline-limit{margin-top:10px;padding:9px 10px;border-radius:9px;background:#fff7e0;color:#6c5100;font:12px/1.5 sans-serif}
...
<div id="offlineModal" class="pmodal-wrap">
  ...
  <div class="pmodal"><button class="pmodal-x" onclick="closeOfflineModal()">✕</button>
    <h3>📥 오프라인 지도</h3>
    <div id="offlineStatus" class="offline-status">저장 상태를 확인하는 중…</div>
    <div id="offlineProgress" class="offline-progress"><i></i></div>
    <div class="offline-actions">
      <button id="offlineSave" class="offline-save" type="button" onclick="saveOfflinePack()">현재 화면 저장</button>
      <button id="offlineUse" class="offline-use" type="button" onclick="useOfflineMap()" disabled>오프라인 지도 보기</button>
      <button id="offlineDelete" class="offline-delete" type="button" onclick="deleteOfflinePack()" disabled>저장본 삭제</button>
    </div>
    <b>사용법</b>
...
// ESRI World Imagery Wayback: 선택한 배포본은 지도 이동 중에도 전 영역에 유지한다.
...
const WB_HIGH_DIFF_RATIO=.005, WB_MEDIUM_DIFF_RATIO=.10;
let _wbVersions=null, _wbLayer=null, _wbTarget=null, _wbLoadSeq=0, _wbMode='full';
const _wbLocalCache={};
const _wbTileBytesCache=new Map(), _wbPixelCache=new Map();
...
function _wbTileUrl(id,d,z){
  const p=_wbTileAt(d,z);
  if(id==='current') return 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/'+z+'/'+p.y+'/'+p.x;
  return 'https://wayback.maptiles.arcgis.com/arcgis/rest/services/World_Imagery/WMTS/1.0.0/default028mm/MapServer/tile/'+id+'/'+z+'/'+p.y+'/'+p.x;
}
function _wbFetchBytes(url){
  if(_wbTileBytesCache.has(url)) return _wbTileBytesCache.get(url);
  const task=fetch(url,{redirect:'follow'}).then(function(r){
    if(!r.ok) throw new Error('Wayback tile '+r.status);
    return r.arrayBuffer();
  }).catch(function(err){ _wbTileBytesCache.delete(url); throw err; });
  _wbTileBytesCache.set(url,task);
  return task;
}
async function _wbDecodePixels(url){
  if(_wbPixelCache.has(url)) return _wbPixelCache.get(url);
  const task=_wbFetchBytes(url).then(async function(bytes){
    const blob=new Blob([bytes]), canvas=document.createElement('canvas');
    canvas.width=64; canvas.height=64;
    let image;
    if(typeof createImageBitmap==='function') image=await createImageBitmap(blob);
    else image=await new Promise(function(resolve,reject){
      const src=URL.createObjectURL(blob), img=new Image();
      img.onload=function(){URL.revokeObjectURL(src);resolve(img);};
      img.onerror=function(){URL.revokeObjectURL(src);reject(new Error('Wayback image decode failed'));};
      img.src=src;
    });
  ...
  }).catch(function(err){ _wbPixelCache.delete(url); throw err; });
  _wbPixelCache.set(url,task);
  return task;
}
function _wbSamplePoints(d){
  return [d,{lat:d.lat+.06,lng:d.lng},{lat:d.lat-.06,lng:d.lng},{lat:d.lat,lng:d.lng+.08},{lat:d.lat,lng:d.lng-.08}];
}
function _wbPixelDiffCounts(a,b){
  if(!a||!b||a.length!==b.length) return null;
  let medium=0, high=0, total=0;
  ...
  return {medium:medium,high:high,total:total};
}
function _wbPixelsMeaningful(a,b){
  const diff=_wbPixelDiffCounts(a,b);
  return !diff||!!diff.total&&(diff.high/diff.total>=WB_HIGH_DIFF_RATIO||diff.medium/diff.total>=WB_MEDIUM_DIFF_RATIO);
}
async function _wbSnapshot(id,points,z){
  return Promise.all(points.map(function(point){return _wbDecodePixels(_wbTileUrl(id,point,z)).catch(function(){return null;});}));
}
function _wbSnapshotMeaningful(a,b){
  let medium=0, high=0, total=0;
  for(let i=0;i<Math.min(a.length,b.length);i++){
    const diff=_wbPixelDiffCounts(a[i],b[i]);
    if(!diff) continue;
    medium+=diff.medium; high+=diff.high; total+=diff.total;
  }
  if(!total) return true;   // 비교 실패 때는 과거 영상을 숨기지 않는 안전한 폴백
  return high/total>=WB_HIGH_DIFF_RATIO||medium/total>=WB_MEDIUM_DIFF_RATIO;
}
async function _wbMapLimit(items,limit,fn){
...
async function _wbLocalVersions(rows,d,onProgress){
  ...
  const z=14, sigs=new Array(rows.length);
  let next=0, done=0;
  async function worker(){
    while(next<rows.length){
      const i=next++, v=rows[i];
      try{
        const b=await _wbFetchBytes(_wbTileUrl(v.id,d,z));
        const h=await crypto.subtle.digest('SHA-256',b);
        sigs[i]=Array.from(new Uint8Array(h)).map(function(x){return x.toString(16).padStart(2,'0');}).join('');
      }catch(e){ sigs[i]=v.id; }
      done++; if(onProgress) onProgress(done,rows.length,'배포본 정리');
    }
  }
  ...
  for(let i=0;i<rows.length;){
    ...
    candidates.push(rows[j]);   // 같은 영상이 이어진 배포본 중 실제로 처음 등장한 날짜
    i=j+1;
  }
  const points=_wbSamplePoints(d), current=await _wbSnapshot('current',points,z);
  done=0;
  const snapshots=await _wbMapLimit(candidates,2,async function(v){
    const result=await _wbSnapshot(v.id,points,z);
    done++; if(onProgress) onProgress(done,candidates.length,'화면 5지점 비교');
    return result;
  });
  const changed=[]; let baseline=current;
  candidates.forEach(function(v,i){
    if(_wbSnapshotMeaningful(baseline,snapshots[i])){changed.push(v);baseline=snapshots[i];}
  });
  const result={rows:changed,excluded:rows.length-changed.length,total:rows.length};
  _wbLocalCache[key]=result;
  return result;
}
function _wbDraw(){
  if(!_wbTarget) return;
  ...
  if(old) old.textContent='과거 · '+(sel.options[sel.selectedIndex]?(sel.options[sel.selectedIndex].dataset.date||sel.options[sel.selectedIndex].text):'선택 시점');
...
function _wbClip(){
  ...
  if(_wbMode!=='split'){ c.style.clip=''; return; }
...
function _wbSetMode(mode){
  _wbMode=mode==='split'?'split':'full';
  document.querySelectorAll('#waybackMode [data-mode]').forEach(function(b){b.classList.toggle('on',b.dataset.mode===_wbMode);});
  const ui=document.getElementById('waybackCompare'); if(ui) ui.classList.toggle('split',_wbMode==='split');
  _wbClip();
}
...
function _openWaybackAtMapCenter(){
  const center=map.getCenter();
  const d={cd:'map-center:'+Number(center.lat).toFixed(4)+','+Number(center.lng).toFixed(4),lat:center.lat,lng:center.lng,lake:'현재 지도 중심'};
  ...
  _mountWaybackOpen();
  try{ localStorage.setItem('mc_basemap','위성지도'); }catch(e){}
  map.attributionControl.removeAttribution('간이 지도 &copy; OpenStreetMap 기여자');
  map.setMaxZoom(SAT_MAXZOOM_E); if(map.getZoom()<11) map.setView([d.lat,d.lng],11); else map.panTo([d.lat,d.lng]);
  const p=document.getElementById('waybackCtl'), sel=document.getElementById('waybackDate'), nm=document.getElementById('waybackName'), note=document.getElementById('waybackNote');
  if(!p||!sel) return;
  ...
  _wbLoadVersions().then(function(rows){
    if(loadSeq!==_wbLoadSeq) return null;
    sel.innerHTML='<option>배포본 분석 중…</option>';
    return _wbLocalVersions(rows,d,function(done,total,stage){ if(loadSeq===_wbLoadSeq) sel.innerHTML='<option>'+stage+'… '+done+'/'+total+'</option>'; });
  }).then(function(result){
    if(loadSeq!==_wbLoadSeq||!result) return;
    if(!result.rows.length){
      if(_wbLayer){map.removeLayer(_wbLayer);_wbLayer=null;}
      sel.innerHTML='<option>현재와 뚜렷하게 다른 영상 없음</option>'; sel.disabled=true;
      if(note) note.textContent='현재 화면 5개 지점을 픽셀 비교했지만 현재와 뚜렷하게 다른 과거 영상을 찾지 못했습니다. 날짜는 촬영일이 아닌 ESRI 배포일입니다.';
      return;
    }
    let saved=''; try{ saved=localStorage.getItem('mc_wayback_id')||''; }catch(e){}
    const hasSaved=result.rows.some(function(v){return v.id===saved;});
    sel.innerHTML=result.rows.map(function(v){return '<option value="'+v.id+'" data-date="'+v.date+'"'+(hasSaved&&v.id===saved?' selected':'')+'>'+v.date+' 배포 · 변화 확인</option>';}).join('');
    if(note) note.textContent='현재 화면 5개 지점을 픽셀 비교해 동일·미세 차이 배포본 '+result.excluded+'개를 제외했습니다. 날짜는 촬영일이 아닌 ESRI 배포일입니다.';
...
let _lastAddr=0;
let _landOwnershipLayer=null;
function clearLandOwnership(){
  if(_landOwnershipLayer){try{map.removeLayer(_landOwnershipLayer);}catch(e){}_landOwnershipLayer=null;}
}
function _landOwnerColor(category){return category==='private'?'#ef6c00':(category==='national'?'#1565c0':(category==='public'?'#00897b':'#757575'));}
async function checkLandOwnership(){
  const a=window._curAddr,btn=document.getElementById('landOwnBtn'),box=document.getElementById('landOwnResult');
  if(!a||!btn||!box)return;
  ...
  try{
    ...
    box.innerHTML='<div class="land-own-card"><div class="land-own-head"><b>토지 정보</b><span class="land-own-badge '+cat+'">'+pmEsc(d.label||'확인 불가')+'</span></div>'
      ...
      +'<a class="land-own-link" target="_blank" rel="noopener" href="https://www.eum.go.kr/web/ar/lu/luLandDet.jsp?isNoScr=script&mode=search&pnu='+encodeURIComponent(pnu)+'">토지이음 상세보기&nbsp;↗</a></div>';
...
function enableAddressPopupDrag(pop){
  const root=pop&&pop.getElement&&pop.getElement(),handle=root&&root.querySelector('.addr-drag-handle');if(!root||!handle)return;
  if(pop._addrDragAbort)pop._addrDragAbort.abort();const aborter=new AbortController(),signal=aborter.signal;pop._addrDragAbort=aborter;
  root.classList.add('addr-popup');
  let drag=null;
  function start(x,y,id){
    const rect=root.getBoundingClientRect(),style=getComputedStyle(root);
    drag={id:id,x:x,y:y,ml:parseFloat(style.marginLeft)||0,mb:parseFloat(style.marginBottom)||0,rect:rect};
  }
  function move(x,y){
    if(!drag)return;const pad=10,dx=Math.max(pad-drag.rect.left,Math.min(innerWidth-pad-drag.rect.right,x-drag.x));
    const dy=Math.max(pad-drag.rect.top,Math.min(innerHeight-pad-drag.rect.bottom,y-drag.y));
    root.style.marginLeft=(drag.ml+dx)+'px';root.style.marginBottom=(drag.mb-dy)+'px';
  }
  handle.addEventListener('mousedown',function(e){if(e.button!==0)return;e.preventDefault();e.stopPropagation();start(e.clientX,e.clientY,'mouse');},{signal:signal});
  document.addEventListener('mousemove',function(e){if(!drag||drag.id!=='mouse')return;e.preventDefault();move(e.clientX,e.clientY);},{signal:signal});
  document.addEventListener('mouseup',function(){if(drag&&drag.id==='mouse')drag=null;},{signal:signal});
  handle.addEventListener('touchstart',function(e){const t=e.changedTouches&&e.changedTouches[0];if(!t)return;e.preventDefault();e.stopPropagation();start(t.clientX,t.clientY,t.identifier);},{passive:false,signal:signal});
  document.addEventListener('touchmove',function(e){if(!drag||drag.id==='mouse')return;const ts=Array.from(e.changedTouches||[]),t=ts.find(function(x){return x.identifier===drag.id;});if(!t)return;e.preventDefault();move(t.clientX,t.clientY);},{passive:false,signal:signal});
  document.addEventListener('touchend',function(e){if(!drag||drag.id==='mouse')return;const ts=Array.from(e.changedTouches||[]);if(ts.some(function(x){return x.identifier===drag.id;}))drag=null;},{signal:signal});
  document.addEventListener('touchcancel',function(){if(drag&&drag.id!=='mouse')drag=null;},{signal:signal});
}
async function showAddress(lat,lng){
  const now=Date.now(); if(now-_lastAddr<800) return; _lastAddr=now;   // 중복 호출 방지
  const grip='<div class="addr-drag-handle" role="button" aria-label="주소 카드 이동"><span class="addr-drag-grip" aria-hidden="true"></span></div>';
  const pop=L.popup({offset:L.point(0,-52),keepInView:false,autoPanPadding:[18,18]}).setLatLng([lat,lng]).setContent(grip+'주소 조회 중…').openOn(map);enableAddressPopupDrag(pop);
  ...
  pop.setContent(h);enableAddressPopupDrag(pop);
}
map.on('popupclose',function(e){clearLandOwnership();if(e.popup&&e.popup._addrDragAbort){e.popup._addrDragAbort.abort();e.popup._addrDragAbort=null;}});
// ---- 어드민: 장소 등록 ----
...
function openCourseComments(key,course,shareId){
  ...
  const actions=[
    ...
    (canAnalyze?'<a class="course-btn ghost" onclick="openOfflineModal(_courseInfoCourse)">📥 오프라인 저장</a>':'')
  ].join('');
  let _cadm='';
...
function _obHasName(ty){ return !!OBS_TYPES[ty]; }
...
function _obDisplayName(o){const t=OBS_TYPES[o&&o.type]||OBS_TYPES['보'];return (o&&o.name&&String(o.name).trim())?String(o.name).trim():t.label;}
function obsIcon(type,name){const t=OBS_TYPES[type]||OBS_TYPES['보'],showName=type==='유명지',raw=(name&&String(name).trim())?String(name).trim():t.label,disp=pmEsc(raw);return L.divIcon({className:'obs-div',html:'<span class="obs-ic '+t.c+(showName?'':' obs-icon-only')+'" aria-label="'+pmEsc(raw)+'">'+t.e+(showName?' '+disp:'')+'</span>',iconSize:null});}
...
function renderObstacle(o){ if(!o||o.lat==null||o.del||(_obAdminOnly(o)&&!isAdmin())) return; _obstacles[o.id]=o;
  ...
  if(!isTouch&&o.type!=='유명지')m.bindTooltip(pmEsc(_obDisplayName(o)),{direction:'top',opacity:.95});
...
function openObsModal(mode,data){ if(!isAdmin()) return;
  ...
  document.getElementById('obBody').innerHTML=
    ...
    +'<div id="obNameRow" style="display:'+(_obHasName(ty)?'block':'none')+'"><div class="sg-label">제목 (선택)</div>'
    +'<input id="obName" maxlength="40" placeholder="예: 도담삼봉 / 한강 여울 / 강변 캠핑사이트" value="'+pmEsc(name)+'"></div>'
...
const WOODEN_CANOE_SVG='<svg viewBox="0 0 42 86" aria-label="위에서 본 우든카누"><path d="M21 2C11 9 5 24 5 43s6 34 16 41c10-7 16-22 16-41S31 9 21 2Z" fill="#58341f" stroke="#fff6e3" stroke-width="2"/><path d="M21 7C14 15 9 28 9 43s5 28 12 36c7-8 12-21 12-36S28 15 21 7Z" fill="#d9a15b"/><path d="M14 16Q19 29 19 43T14 70M28 16Q23 29 23 43T28 70M21 8V78" fill="none" stroke="#f4d397" stroke-width="1.2" opacity=".85"/><path d="M11 28H31M10 43H32M11 58H31" stroke="#4c2d1b" stroke-width="3" stroke-linecap="round"/><rect x="15" y="32" width="12" height="5" rx="1" fill="#6d4024"/><rect x="15" y="49" width="12" height="5" rx="1" fill="#6d4024"/></svg>';
const MODEL_Y_SVG='<svg viewBox="0 0 52 82" aria-label="위에서 본 Tesla Model Y"><rect x="2" y="18" width="5" height="18" rx="2.5" fill="#172a36"/><rect x="45" y="18" width="5" height="18" rx="2.5" fill="#172a36"/><rect x="2" y="49" width="5" height="17" rx="2.5" fill="#172a36"/><rect x="45" y="49" width="5" height="17" rx="2.5" fill="#172a36"/><path d="M26 2C15 2 10 8 8 17L6 31v24l3 17c1 5 7 8 17 8s16-3 17-8l3-17V31l-2-14C42 8 37 2 26 2Z" fill="#f8fbfd" stroke="#17384b" stroke-width="2"/><path d="M15 18Q26 10 37 18L40 31H12Z" fill="#243f50"/><path d="M12 35H40V56H12Z" fill="#9dc9dc"/><path d="M14 59H38L36 70Q26 74 16 70Z" fill="#314d5c"/><path d="M11 14Q26 7 41 14" fill="none" stroke="#a8e2ff" stroke-width="2.5" stroke-linecap="round"/><path d="M13 73Q26 77 39 73" fill="none" stroke="#ef5350" stroke-width="2.5" stroke-linecap="round"/><path d="M8 30L3 26M44 30L49 26" stroke="#f8fbfd" stroke-width="3" stroke-linecap="round"/></svg>';
const PERSON_SVG='<svg viewBox="0 0 40 48" aria-label="보행자"><circle cx="20" cy="8" r="6" fill="#fff" stroke="#314b5b" stroke-width="2"/><path d="M20 15L18 29L10 42M18 29L28 41M19 19L9 28M20 20L30 26" fill="none" stroke="#fff" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 15L18 29L10 42M18 29L28 41M19 19L9 28M20 20L30 26" fill="none" stroke="#314b5b" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function _addRoadviewFeature(f){
  ...
  const dot=L.circleMarker([lat,lng],{radius:5,color:'#fff',weight:2,fillColor:'#00acc1',fillOpacity:1});
  [ln,dot].forEach(function(layer){layer.bindTooltip('🛣️ '+name,{sticky:true,direction:'top',opacity:0.95});layer.on('click',function(){openRoadview(lat,lng,name,f.properties.id);});});
  roadviewLayer.addLayer(L.layerGroup([ln,dot]));
}
...
// ---- 전국 다이소 지점(공식 매장검색 스냅샷, 기본 ON·줌≥10 지연 로드) ----
const DAISO_MARKER_SVG='<svg viewBox="0 0 52 52" aria-label="다이소"><circle cx="26" cy="26" r="23" fill="#fff" stroke="#e31b3d" stroke-width="4"/><circle cx="39" cy="12" r="7" fill="#e31b3d" stroke="#fff" stroke-width="2"/><path d="M18 17h14v5H23v9h9v5H18ZM35 15h5v7h5v5h-5v11h-5Z" fill="#e31b3d"/></svg>';
const daisoLayer=L.layerGroup().addTo(map);
let _daisoFeatures=null,_daisoMeta={},_daisoLoading=null;
const Z_DAISO=10,Z_DAISO_INDIVIDUAL=14;
function _daisoIcon(){return L.divIcon({className:'daiso-div',html:'<span class="daiso-pin">'+DAISO_MARKER_SVG+'</span>',iconSize:[34,34],iconAnchor:[17,17]});}
function _daisoClusterIcon(n){return L.divIcon({className:'daiso-div',html:'<span class="daiso-cluster">'+n+'</span>',iconSize:[38,38],iconAnchor:[19,19]});}
function _daisoPopup(f){
  const p=f.properties||{},c=f.geometry.coordinates,n='다이소 '+(p.name||'매장'),phone=p.phone?'<br><a href="tel:'+String(p.phone).replace(/[^0-9+]/g,'')+'">'+pmEsc(p.phone)+'</a>':'',hours=p.hours?'<br><small>영업시간 '+pmEsc(p.hours)+'</small>':'',asof=_daisoMeta.asOf?'<br><small style="color:#889">공식 매장검색 · '+pmEsc(_daisoMeta.asOf)+' 기준</small>':'';
  return '<b>'+pmEsc(n)+'</b><br>'+pmEsc(p.address||'')+phone+hours+asof+extLinks(c[1],c[0],n,false)+'<br><a href="https://www.daiso.co.kr/cs/shop" target="_blank" rel="noopener">공식 매장정보 확인</a>';
}
function _addDaisoMarker(f){
  const c=f.geometry.coordinates,p=f.properties||{},m=L.marker([c[1],c[0]],{icon:_daisoIcon(),pane:'markerPane'}).bindTooltip(pmEsc('다이소 '+(p.name||'매장')),{direction:'top',offset:[0,-16]});
  m.bindPopup(_daisoPopup(f),{minWidth:220,maxWidth:310}).addTo(daisoLayer);
}
function _renderDaiso(){
  daisoLayer.clearLayers();if(!_daisoFeatures||!map.hasLayer(daisoLayer)||map.getZoom()<Z_DAISO)return;
  const bounds=map.getBounds().pad(.08),visible=_daisoFeatures.filter(function(f){const c=f.geometry.coordinates;return bounds.contains([c[1],c[0]]);});
  if(map.getZoom()>=Z_DAISO_INDIVIDUAL){visible.forEach(_addDaisoMarker);return;}
  ...
  Object.keys(cells).forEach(function(key){const fs=cells[key];if(fs.length===1){_addDaisoMarker(fs[0]);return;}const ll=fs.map(function(f){const c=f.geometry.coordinates;return[c[1],c[0]];}),center=L.latLngBounds(ll).getCenter(),m=L.marker(center,{icon:_daisoClusterIcon(fs.length),pane:'markerPane'}).bindTooltip('다이소 '+fs.length+'곳',{direction:'top'});m.on('click',function(){const b=L.latLngBounds(ll);if(b.getNorthEast().equals(b.getSouthWest()))map.setView(center,Math.min(15,map.getZoom()+2));else map.fitBounds(b.pad(.2),{maxZoom:14});});m.addTo(daisoLayer);});
}
function _loadDaiso(){
  if(_daisoFeatures)return Promise.resolve(_daisoFeatures);if(_daisoLoading)return _daisoLoading;
  _daisoLoading=fetch('./daiso_stores.geojson?v='+DATAVER.daiso).then(function(r){if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(function(fc){_daisoFeatures=fc.features||[];_daisoMeta=fc.metadata||{};return _daisoFeatures;}).finally(function(){_daisoLoading=null;});return _daisoLoading;
}
function _daisoZoomGate(){if(!map.hasLayer(daisoLayer)||map.getZoom()<Z_DAISO){daisoLayer.clearLayers();return;}_loadDaiso().then(_renderDaiso).catch(function(){_offToast('다이소 지점 데이터를 불러오지 못했습니다');});}
map.on('overlayadd',function(e){if(e&&e.layer===daisoLayer)_daisoZoomGate();});
...
let _hanaroFeatures=null,_hanaroMeta={},_hanaroLoading=null;
...
function _loadHanaro(){if(_hanaroFeatures)return Promise.resolve(_hanaroFeatures);if(_hanaroLoading)return _hanaroLoading;_hanaroLoading=fetch('./hanaro_stores.geojson?v='+DATAVER.hanaro).then(function(r){if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(function(fc){_hanaroFeatures=fc.features||[];_hanaroMeta=fc.metadata||{};return _hanaroFeatures;}).finally(function(){_hanaroLoading=null;});return _hanaroLoading;}
...
function _lcTag(group){return '<i class="lc-group-tag" data-lc-group="'+group+'"></i>';}
_ov[_lcTag('safety')+_sw('rgba(229,57,53,.45)')+'상수원보호'] = _protectPH;
_ov[_lcTag('safety')+_sw('rgba(255,152,0,.5)')+'수상레저금지'] = _wlzPH;
_ov[_lcTag('safety')+'<span class="rv-sw">🛟</span>물놀이 관리지역'] = _waterplayPH;
_ov[_lcTag('canoe')+'<span class="sw sw-course"></span>🛶 카누잉 코스'] = allCoursesGroup;   // 코스 전체 단일 토글
_ov[_lcTag('canoe')+_sw('#ec407a')+'카누명소'] = famousLayer;
_ov[_lcTag('canoe')+_sw('#2196f3')+'런칭/랜딩'] = canoeLayer;
_ov[_lcTag('canoe')+'<span class="rv-sw">⚠️</span>지형지물·보'] = obstacleLayer;     // 기본 ON, 줌≥12 표시
_ov[_lcTag('canoe')+'<span class="rv-sw">🛣️</span>로드뷰 구간'] = roadviewLayer;   // 기본 OFF
_ov[_lcTag('water')+'<span class="rv-sw">💧</span>하천'] = waterLevelLayer;        // 수위 묶음, 기본 OFF, 줌≥12 표시
_ov[_lcTag('water')+'<span class="rv-sw">🏞️</span>호수·댐'] = damLevelLayer;      // 수위 묶음, 기본 OFF, 줌≥10 표시
_ov[_lcTag('water')+'<span class="rv-sw">📹</span>CCTV'] = cctvLayer;             // 수위 묶음, 기본 OFF, 줌≥12 표시
...
const RIVER_SKY_COLOR='#bdeeff',_riverWaveMarks=L.layerGroup();_riverAdminGroup.addLayer(_riverWaveMarks);
...
function _riverWaveMarker(ll,angle,pane){
  const svg='<span class="river-wave-mark" style="transform:rotate('+(angle-90).toFixed(1)+'deg)"><svg viewBox="0 0 20 12" aria-hidden="true"><path d="M1 7Q5.5 2 10 7T19 7" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round"/></svg></span>';
  return L.marker(ll,{pane:pane||'overlayPane',interactive:false,keyboard:false,icon:L.divIcon({className:'river-wave-icon',html:svg,iconSize:[20,12],iconAnchor:[10,6]})});
}
function riverWaveLayer(coords,pane,limit){
  const out=L.layerGroup();if(!coords||coords.length<2)return out;
  const seg=[],dist=[];let total=0;
  for(let i=1;i<coords.length;i++){const d=map.distance(coords[i-1],coords[i]);if(!isFinite(d)||d<=0)continue;seg.push([coords[i-1],coords[i],d]);total+=d;dist.push(total);}
  if(!seg.length||total<=0)return out;const count=Math.max(1,Math.min(limit||5,Math.round(total/9000)));
  for(let n=1;n<=count;n++){const target=total*n/(count+1);let i=0;while(i<dist.length-1&&dist[i]<target)i++;const prev=i?dist[i-1]:0,s=seg[i],f=Math.max(0,Math.min(1,(target-prev)/s[2])),ll=[s[0][0]+(s[1][0]-s[0][0])*f,s[0][1]+(s[1][1]-s[0][1])*f];_riverWaveMarker(ll,_courseBearing(s[0],s[1]),pane).addTo(out);}
  return out;
}
function _refreshRiverWaveMarks(){
  _riverWaveMarks.clearLayers();if(!_riverFeatures.length||!map.hasLayer(_riverAdminGroup)||map.getZoom()<9)return;
  const bounds=map.getBounds().pad(.08),seen={},limit=map.getZoom()>=11?120:70,ordered=_riverFeatures.filter(function(f){return (f.properties||{}).kind==='river';}).concat(map.getZoom()>=11?_riverFeatures.filter(function(f){return (f.properties||{}).kind==='stream';}):[]);let count=0;
  for(let n=0;n<ordered.length&&count<limit;n++){const cs=(ordered[n].geometry||{}).coordinates||[];if(cs.length<2)continue;const tries=Math.min(3,Math.max(1,Math.floor(cs.length/18)));for(let j=1;j<=tries&&count<limit;j++){const i=Math.max(1,Math.min(cs.length-1,Math.round((cs.length-1)*j/(tries+1)))),a=cs[i-1],b=cs[i],ll=[b[1],b[0]];if(!bounds.contains(ll))continue;const p=map.latLngToContainerPoint(ll),key=Math.floor(p.x/34)+':'+Math.floor(p.y/26);if(seen[key])continue;seen[key]=1;_riverWaveMarker(ll,_courseBearing([a[1],a[0]],ll)).addTo(_riverWaveMarks);count++;}}
}
...
function _riverZoomGate(){ if(!_riverStream||!map.hasLayer(_riverAdminGroup)) return; const show=map.getZoom()>=11; if(show&&!_riverAdminGroup.hasLayer(_riverStream))_riverAdminGroup.addLayer(_riverStream); else if(!show&&_riverAdminGroup.hasLayer(_riverStream))_riverAdminGroup.removeLayer(_riverStream);_refreshRiverWaveMarks(); }
function _loadAdminRivers(){
  ...
  _riverLoading=fetch('rivers.geojson?v='+DATAVER.rivers).then(function(r){if(!r.ok)throw 0;return r.json();}).then(function(fc){
    ...
    const opts=function(kind){return {filter:function(f){return (f.properties||{}).kind===kind;},style:kind==='river'?{color:RIVER_SKY_COLOR,weight:6.5,opacity:.96}:{color:RIVER_SKY_COLOR,weight:4.5,opacity:.88},onEachFeature:function(f,l){const n=(f.properties||{}).name;if(n)l.bindTooltip(pmEsc(n),{sticky:true,direction:'top'});}};};
...
function _syncAdminRiverLayer(on){
  ...
  if(on&&!_riverControlAdded){_layerControl.addOverlay(_riverAdminGroup,_sw(RIVER_SKY_COLOR)+'주요 강·하천 <small>(관리자)</small>');_riverControlAdded=true;}
...
map.on('moveend',_refreshRiverWaveMarks);
...
function focusRiverSearch(x,keepUrl){
  ...
  (x.features||[]).forEach(function(f){const ll=f.geometry.coordinates.map(function(c){return [c[1],c[0]];});if(ll.length<2)return;if(!anchor)anchor=ll[Math.floor(ll.length/2)];L.polyline(ll,{pane:'riverSearchPane',color:'#55c9ef',weight:10,opacity:.82,interactive:false}).addTo(_riverSearchFocus);L.polyline(ll,{pane:'riverSearchPane',color:RIVER_SKY_COLOR,weight:7.5,opacity:1,interactive:false}).addTo(_riverSearchFocus);riverWaveLayer(ll,'riverSearchPane',5).addTo(_riverSearchFocus);bounds.extend(ll);});
...
// 과거 영상은 범례의 위성지도 행에서 현재 지도 중심을 기준으로 연다.
function _mountWaybackOpen(){
  const c=_layerControl&&_layerControl.getContainer(); if(!c) return;
  const base=c.querySelector('.leaflet-control-layers-base'); if(!base) return;
  const satLabel=Array.from(base.querySelectorAll('label')).find(function(x){return (x.textContent||'').indexOf('위성지도')>=0;});
  if(!satLabel||satLabel.querySelector('.wayback-open'))return;
  satLabel.classList.add('sat-base-row');
  const open=L.DomUtil.create('button','wayback-open',satLabel);open.type='button';open.innerHTML='🕘 과거';open.title='현재 지도 중심의 과거 위성사진 보기';open.setAttribute('aria-label',open.title);
  open.onclick=function(e){e.preventDefault();e.stopPropagation();_openWaybackAtMapCenter();return false;};
}
(function(){
  const c=_layerControl&&_layerControl.getContainer(); if(!c) return;
  const base=c.querySelector('.leaflet-control-layers-base'); if(!base) return;
  _mountWaybackOpen();
  new MutationObserver(_mountWaybackOpen).observe(base,{childList:true,subtree:true});
  ...
  p.innerHTML='<div class="wayback-head"><span>과거 위성 · <span id="waybackName"></span></span><button id="waybackClose" type="button" title="과거 영상 닫기">×</button></div>'
    ...
    +'<div class="wayback-mode" id="waybackMode" aria-label="과거 위성영상 표시 방식"><button type="button" data-mode="full" class="on">과거 전체</button><button type="button" data-mode="split">현재와 비교</button></div>'
    +'<small class="wayback-note" id="waybackNote">현재 화면 5개 지점의 실제 픽셀 변화를 확인해 과거 영상을 선별합니다. 날짜는 촬영일이 아닌 ESRI 배포일입니다.</small>';
  ...
  p.querySelectorAll('#waybackMode [data-mode]').forEach(function(b){L.DomEvent.on(b,'click',function(e){L.DomEvent.stop(e);_wbSetMode(b.dataset.mode);gaEvent('wayback_mode_'+b.dataset.mode);});});
...
loadCourses();
// ---- 코스 URL 공유 ----
function courseShareUrl(idStr){ return WORKER_URL.replace(/\/+$/,'')+'/c/'+encodeURIComponent(String(idStr||'')); }
function _courseShareInfo(idStr,name,km){
  ...
  return {name:String(name||c.name||'카누맵 코스').trim()||'카누맵 코스',dist:dist,course:c};
}
const _coursePreviewDone={};
let _coursePreviewRiverPromise=null;
function _cpRound(ctx,x,y,w,h,r){ctx.beginPath();ctx.moveTo(x+r,y);ctx.arcTo(x+w,y,x+w,y+h,r);ctx.arcTo(x+w,y+h,x,y+h,r);ctx.arcTo(x,y+h,x,y,r);ctx.arcTo(x,y,x+w,y,r);ctx.closePath();}
function _cpText(ctx,text,x,y,maxWidth,size,weight,color){
  const family='Apple SD Gothic Neo, Pretendard, Noto Sans KR, sans-serif';let s=size,t=String(text||'');ctx.font=weight+' '+s+'px '+family;
  while(s>15&&ctx.measureText(t).width>maxWidth){s--;ctx.font=weight+' '+s+'px '+family;}
  if(ctx.measureText(t).width>maxWidth){while(t.length>1&&ctx.measureText(t+'…').width>maxWidth)t=t.slice(0,-1);t+='…';}
  ctx.fillStyle=color;ctx.fillText(t,x,y);return t;
}
function _cpLabels(name){
  const raw=String(name||'카누맵 코스').trim(),dash=raw.indexOf(' - ');let title=dash>0?raw.slice(0,dash):raw,route=dash>0?raw.slice(dash+3):'';
  const ends=route.split(/\s+(?:~|→)\s+/);return {title:title,start:ends[0]||'코스 출발점',end:ends[1]||'코스 도착점'};
}
function _cpProjection(coords,frame){
  const merc=function(p){const lat=Math.max(-85,Math.min(85,+p[0]))*Math.PI/180;return [(+p[1])*Math.PI/180,Math.log(Math.tan(Math.PI/4+lat/2))];};
  const ps=coords.map(merc),xs=ps.map(function(p){return p[0];}),ys=ps.map(function(p){return p[1];}),minX=Math.min.apply(null,xs),maxX=Math.max.apply(null,xs),minY=Math.min.apply(null,ys),maxY=Math.max.apply(null,ys),dx=Math.max(1e-7,maxX-minX),dy=Math.max(1e-7,maxY-minY),scale=Math.min((frame.w-68)/dx,(frame.h-48)/dy),cx=(minX+maxX)/2,cy=(minY+maxY)/2;
  return function(p){const m=merc(p);return [frame.x+frame.w/2+(m[0]-cx)*scale,frame.y+frame.h/2-(m[1]-cy)*scale];};
}
function _cpRouteContext(coords){
  if(!_coursePreviewRiverPromise)_coursePreviewRiverPromise=fetch('rivers.geojson?v='+DATAVER.rivers).then(function(r){return r.ok?r.json():{features:[]};}).catch(function(){return {features:[]};});
  return _coursePreviewRiverPromise.then(function(fc){
    const lats=coords.map(function(p){return +p[0];}),lngs=coords.map(function(p){return +p[1];}),s=Math.min.apply(null,lats),n=Math.max.apply(null,lats),w=Math.min.apply(null,lngs),e=Math.max.apply(null,lngs),py=Math.max(.025,(n-s)*.35),px=Math.max(.025,(e-w)*.35);
    return (fc.features||[]).filter(function(f){const cs=(f.geometry&&f.geometry.coordinates)||[];return cs.some(function(p){return p[0]>=w-px&&p[0]<=e+px&&p[1]>=s-py&&p[1]<=n+py;});}).slice(0,45).map(function(f){return {kind:(f.properties||{}).kind||'',coords:(f.geometry.coordinates||[]).map(function(p){return [p[1],p[0]];})};});
  });
}
function _drawCoursePreview(course,context){
  const coords=(course&&course.coords||[]).filter(function(p){return p&&isFinite(p[0])&&isFinite(p[1]);});if(coords.length<2)return '';
  ...
  ctx.fillStyle='#edf2e6';ctx.fillRect(0,0,1200,630);ctx.save();ctx.strokeStyle='#e1ead8';ctx.lineWidth=1;ctx.globalAlpha=.75;for(let x=-600;x<1400;x+=42){ctx.beginPath();ctx.moveTo(x,630);ctx.lineTo(x+364,0);ctx.stroke();}ctx.restore();
  const frame={x:500,y:38,w:658,h:554},project=_cpProjection(coords,frame);ctx.save();ctx.beginPath();ctx.rect(frame.x,frame.y,frame.w,frame.h);ctx.clip();
  (context||[]).forEach(function(line,i){const ps=line.coords.map(project);if(ps.length<2)return;ctx.beginPath();ps.forEach(function(p,j){if(j)ctx.lineTo(p[0],p[1]);else ctx.moveTo(p[0],p[1]);});ctx.strokeStyle=RIVER_SKY_COLOR;ctx.globalAlpha=line.kind==='river'?.84:.58;ctx.lineWidth=line.kind==='river'?6:2.5;ctx.lineJoin='round';ctx.lineCap='round';ctx.stroke();});
  const route=coords.map(project);function stroke(color,width){ctx.beginPath();route.forEach(function(p,i){if(i)ctx.lineTo(p[0],p[1]);else ctx.moveTo(p[0],p[1]);});ctx.strokeStyle=color;ctx.globalAlpha=1;ctx.lineWidth=width;ctx.lineCap='round';ctx.lineJoin='round';ctx.stroke();}
  stroke('#ffffff',14);stroke(validCourseColor(course.color)||'#d500f9',8);ctx.restore();
  function marker(p,color,label,side){ctx.save();ctx.shadowColor='rgba(21,45,37,.22)';ctx.shadowBlur=10;ctx.shadowOffsetY=3;ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(p[0],p[1],17,0,Math.PI*2);ctx.fill();ctx.shadowColor='transparent';ctx.fillStyle=color;ctx.beginPath();ctx.arc(p[0],p[1],11,0,Math.PI*2);ctx.fill();ctx.font='800 19px Apple SD Gothic Neo, Pretendard, sans-serif';ctx.textAlign=side==='right'?'left':'right';ctx.lineWidth=5;ctx.strokeStyle='#fff';ctx.strokeText(label,p[0]+(side==='right'?25:-25),p[1]+6);ctx.fillStyle='#14231e';ctx.fillText(label,p[0]+(side==='right'?25:-25),p[1]+6);ctx.restore();}
  marker(route[0],'#13a26f','출발','right');marker(route[route.length-1],'#ef5b5b','도착','left');
  ctx.save();ctx.shadowColor='rgba(21,45,37,.2)';ctx.shadowBlur=18;ctx.shadowOffsetY=5;ctx.fillStyle='#fff';_cpRound(ctx,42,42,430,546,26);ctx.fill();ctx.shadowColor='transparent';ctx.strokeStyle='#cbd8cf';ctx.lineWidth=1;ctx.stroke();ctx.restore();
  const labels=_cpLabels(course.name);_cpText(ctx,'카누맵 · 추천 코스',76,102,362,20,'800','#5d6d65');_cpText(ctx,labels.title,76,161,362,40,'900','#14231e');ctx.strokeStyle='#cbd8cf';ctx.beginPath();ctx.moveTo(76,192);ctx.lineTo(438,192);ctx.stroke();
  _cpText(ctx,'출발',76,239,362,18,'700','#5d6d65');_cpText(ctx,labels.start,76,273,362,25,'850','#14231e');_cpText(ctx,'도착',76,326,362,18,'700','#5d6d65');_cpText(ctx,labels.end,76,360,362,25,'850','#14231e');
  ctx.fillStyle='#e1ead8';_cpRound(ctx,76,407,362,88,18);ctx.fill();_cpText(ctx,'전체 거리',100,442,314,17,'750','#5d6d65');_cpText(ctx,(Number(course.km)||0).toFixed(2).replace(/\.00$/,'')+' km',100,477,314,32,'900','#14231e');_cpText(ctx,'전체 코스를 지도에서 확인하세요 →',76,552,362,19,'850',validCourseColor(course.color)||'#d500f9');
  ctx.fillStyle='#fff';_cpRound(ctx,1026,548,116,42,21);ctx.fill();ctx.strokeStyle='#cbd8cf';ctx.stroke();ctx.textAlign='center';ctx.font='900 18px Apple SD Gothic Neo, Pretendard, sans-serif';ctx.fillStyle='#14231e';ctx.fillText('카누맵',1084,576);ctx.textAlign='left';
  return canvas.toDataURL('image/jpeg',.86);
}
async function _prepareCoursePreview(idStr,course){
  const id=String(idStr||''),c=course||{};if(!/^(?:k[0-9]{8,24}|[0-9]{1,12})$/.test(id)||!c.coords||c.coords.length<2||!_canEditCourse(c,id))return false;
  const sig=[c.name,c.km,c.color,c.coords.length,c.coords[0],c.coords[c.coords.length-1]].join('|');if(_coursePreviewDone[id]===sig)return true;
  try{const context=await _cpRouteContext(c.coords),img=_drawCoursePreview(c,context);if(!img)return false;const u=getUser()||{},body={id:id,img:img};if(isAdmin())body.adminKey=adminKey();else{body.uid=u.uid||'';body.tok=u.tok||'';}const r=await fetch(fapi('/course-preview'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!r.ok)return false;_coursePreviewDone[id]=sig;return true;}catch(e){return false;}
}
function shareCourse(idStr,name,km){ const u=courseShareUrl(idStr),info=_courseShareInfo(idStr,name,km),lines=['🛶 코스명: '+info.name];if(info.dist)lines.push('📏 거리: 약 '+info.dist+'km');lines.push('🔗 '+u);const full=lines.join('\n');gaEvent('course_share');_prepareCoursePreview(idStr,info.course);
...
function _localSearch(q){
  ...
  // 직접 등록한 장소·코스·지형지물이 항상 먼저, 공용 호수 색인은 그 다음에 둔다.
...
function _orderedLocalSearch(q,riverLocal,roadLocal){
  return _localSearch(q).concat(riverLocal||[],roadLocal||[]);
}
document.getElementById('srchForm').addEventListener('submit', async (ev)=>{
  ...
  const local=_orderedLocalSearch(q,riverLocal,roadLocal);   // 직접 등록한 장소·코스 → 호수 → 하천·도로
...
let _lastMeasureShare=null,_lastMeasureShortUrl='';
...
function finishMeasure(){
  ...
  _lastMeasureShare={coords:cc,km:Math.round(km*100)/100}; _prepareMeasureShare(_lastMeasureShare);
...
function _shortCoursePlace(label,addr){
  label=(label||'').trim();
  function localAddress(value){let p=(value||'').trim().split(/\s+/).filter(Boolean),province=-1;for(let j=0;j<p.length;j++)if(/(특별자치도|도)$/.test(p[j]))province=j;if(province>=0)p=p.slice(province+1);else{const i=p.findIndex(function(x){return /(특별시|광역시|특별자치시|시|군|구)$/.test(x);});if(i>=0)p=p.slice(i);}p=p.filter(function(x,i){return !i||x!==p[i-1];});return p.join(' ');}
  function regionText(value){const p=localAddress(value).split(/\s+/).filter(Boolean);if(!p.length)return '';const r=[p[0]];if(/시$/.test(p[0])&&p[1]&&/구$/.test(p[1]))r.push(p[1]);return r.join(' ');}
  if(label){const halves=label.split(/\s+-\s+/),raw=(halves[0]||'').replace(/\([^)]*\)/g,'').trim(),region=regionText(raw),place=(halves.length>1?halves.slice(1).join(' - '):label).replace(/^(춘천|양구|곡성|구례|연천|홍천|인제|화천)\s*[()]/,'').trim();if(/(?:특별자치도|도|특별시|광역시|특별자치시|\S+[시군구])\s/.test(place))return localAddress(place);return (region&&place.indexOf(region)!==0?region+' ':'')+place;}
  return localAddress(addr);
}
...
async function doSaveCourse(){
  ...
  try{
    ...
    if(r.ok){
      const editing=(_cmMode==='edit'||_cmMode==='editstatic');let previewId='',previewCourse=null;
      if(_cmMode==='editstatic'&&_cmCourse){_cmCourse.name=name;_cmCourse.color=color;previewId=String(_cmCourse.id);previewCourse=_cmCourse;}
      else if(_cmMode==='edit'&&_cmCourse){_cmCourse.name=name;_cmCourse.color=color;previewId='k'+_cmCourse.id;previewCourse=_cmCourse;}
      else {previewCourse=saved.course||{id:Date.now(),name:name,color:color,coords:_lastCourse.coords,km:_lastCourse.km,segments:_lastCourse.segments||[],owner:isAdmin()?'admin':u.uid};previewId='k'+previewCourse.id;}
      msg.textContent='공유 미리보기 생성 중…';await _prepareCoursePreview(previewId,previewCourse);
      gaEvent(editing?'course_edit':'course_add'); bumpCourseVersion(); closeCourseModal(); map.closePopup();
      if(editing){ location.reload(); }
      else { renderKVCourse(previewCourse); _lastCourse=null; }
...
async function waterRoute(p1,p2){
  ...
  const nodes={}, adj={}, nodeNames={}, graphEdges=[], toR=Math.PI/180;
  ...
  // 춘천호 오월1교 일대의 넓은 수면 폴리곤이 둑을 물로 오인하므로 임의 직선 단축만 막는다.
  // OSM waterway 중심선은 오월교 아래 실제 수로를 따르므로 그래프 간선에는 이 경계를 적용하지 않는다.
  const navigationBarriers=[[[37.9905,127.6428],[37.9765,127.6575]]];
  function _orient(a,b,c){return (b[1]-a[1])*(c[0]-a[0])-(b[0]-a[0])*(c[1]-a[1]);}
  function _segCross(a,b,c,d){const ab1=_orient(a,b,c),ab2=_orient(a,b,d),cd1=_orient(c,d,a),cd2=_orient(c,d,b);return ((ab1<=0&&ab2>=0)||(ab1>=0&&ab2<=0))&&((cd1<=0&&cd2>=0)||(cd1>=0&&cd2<=0));}
  function crossesNavigationBarrier(a,b){return navigationBarriers.some(function(line){for(let i=1;i<line.length;i++)if(_segCross(a,b,line[i-1],line[i]))return true;return false;});}
  ...
  for(const el of (j.elements||[])){
    ...
    for(const p of el.geometry){ const k=key(p.lat,p.lon); if(wayName)(nodeNames[k]||(nodeNames[k]={}))[wayName]=((nodeNames[k]||{})[wayName]||0)+1;if(prev&&prev!==k){ const d=hav(nodes[prev],nodes[k]);
      (adj[prev]=adj[prev]||[]).push([k,d]); (adj[k]=adj[k]||[]).push([prev,d]); graphEdges.push([prev,k]); } prev=k; } }
  ...
  const wetMemo={};
  function plausibleWater(pt){const mk=pt[0].toFixed(4)+','+pt[1].toFixed(4);if(mk in wetMemo)return wetMemo[mk];if(inWater(pt)||passableStructure(pt))return wetMemo[mk]=true;let d=1e18;for(const x of waterOuter)d=Math.min(d,edgeDist(pt,x));for(const x of waterInner)d=Math.min(d,edgeDist(pt,x));return wetMemo[mk]=d<=35;}
  function waterChord(a,b){const d=hav(a,b);if(d>450||crossesNavigationBarrier(a,b))return false;const steps=Math.max(2,Math.ceil(d/60));let plausible=0,dryRun=0,maxDry=0;for(let i=0;i<=steps;i++){const t=i/steps,p=[a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t];if(plausibleWater(p)){plausible++;dryRun=0;}else{dryRun++;maxDry=Math.max(maxDry,dryRun);}}return plausible/(steps+1)>=.9&&maxDry*60<=120;}
  function shortcutWaterPath(raw){if(raw.length<3||!waterOuter.length)return raw;const out=[raw[0]];let i=0;while(i<raw.length-1){let good=i+1,bad=raw.length,step=2;while(i+step<raw.length){if(waterLineClear(raw[i],raw[i+step])){good=i+step;step*=2;}else{bad=i+step;break;}}if(bad===raw.length&&good<raw.length-1&&waterLineClear(raw[i],raw[raw.length-1]))good=raw.length-1;else{let lo=good+1,hi=Math.min(bad-1,raw.length-1);while(lo<=hi){const mid=(lo+hi)>>1;if(waterLineClear(raw[i],raw[mid])){good=mid;lo=mid+1;}else hi=mid-1;}}out.push(raw[good]);i=good;}return out;}
  function waterLineClear(a,b){
    if(crossesNavigationBarrier(a,b))return false;
    const d=hav(a,b),steps=Math.max(2,Math.ceil(d/8));let dry=0;
  ...
  function waterSurfacePath(a,b){
    ...
    while(heap.size()){
      ...
      for(const dc of dirs){const r=ur+dc[0],c=uc+dc[1];if(r<0||r>=rows||c<0||c>=cols)continue;const v=r*cols+c;if(!wet(v))continue;
        ...
        if(!waterLineClear(up,point(v)))continue;
  ...
  function edgeProjection(origin,edge){
    const a=nodes[edge[0]],b=nodes[edge[1]],c=Math.cos(origin[0]*toR),ax=(a[1]-origin[1])*111320*c,ay=(a[0]-origin[0])*110540,bx=(b[1]-origin[1])*111320*c,by=(b[0]-origin[0])*110540,dx=bx-ax,dy=by-ay;
    const t=Math.max(0,Math.min(1,-(ax*dx+ay*dy)/((dx*dx+dy*dy)||1))),p=[a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t];
    return {edge:edge,t:t,p:p,dist:hav(origin,p),edgeId:edge[0]<edge[1]?edge[0]+'|'+edge[1]:edge[1]+'|'+edge[0]};
  }
  function attachProjected(pt,tag){
  ...
  function nearestWet(pt){
    const projected=attachProjected(pt,pt===p1?'@route-start':'@route-end');if(projected)return projected;
  ...
  if(near1.projection&&near2.projection&&near1.edgeId===near2.edgeId&&s1!==s2){const d=hav(nodes[s1],nodes[s2]);(adj[s1]=adj[s1]||[]).push([s2,d]);(adj[s2]=adj[s2]||[]).push([s1,d]);}
...
// ---- 내 위치 ----
let _locMarker=null,_locCircle=null,_locWatching=false,_locHasFix=false,_locMode='person',_locModePending='',_locModeHits=0,_locLastEvent=null;
...
function _locIcon(mode){
  const svg=mode==='car'?MODEL_Y_SVG:(mode==='canoe'?WOODEN_CANOE_SVG:PERSON_SVG);
  return L.divIcon({className:'loc-transport',html:'<span class="loc-transport-in '+mode+'">'+svg+'</span>',iconSize:[46,42],iconAnchor:[23,21]});
}
function _locPointSegmentM(p,s){
  const cos=Math.cos(p.lat*Math.PI/180),ax=(s[1]-p.lng)*111320*cos,ay=(s[0]-p.lat)*110540,bx=(s[3]-p.lng)*111320*cos,by=(s[2]-p.lat)*110540,dx=bx-ax,dy=by-ay,t=Math.max(0,Math.min(1,-(ax*dx+ay*dy)/((dx*dx+dy*dy)||1))),x=ax+t*dx,y=ay+t*dy;return Math.sqrt(x*x+y*y);
}
function _locNearbySegments(fc,c,pad){
  const out=[],s=c.lat-pad,n=c.lat+pad,w=c.lng-pad,e=c.lng+pad;
  (fc&&fc.features||[]).forEach(function(f){const g=f.geometry||{},lines=g.type==='LineString'?[g.coordinates]:(g.type==='MultiLineString'?g.coordinates:[]);lines.forEach(function(line){for(let i=1;i<line.length;i++){const a=line[i-1],b=line[i];if(Math.max(a[1],b[1])<s||Math.min(a[1],b[1])>n||Math.max(a[0],b[0])<w||Math.min(a[0],b[0])>e)continue;out.push([a[1],a[0],b[1],b[0]]);}});});
  return out;
}
function _buildLocSurfaceWindow(c){
  if(!_locSurfaceData)return null;
  _locSurfaceWindow={center:L.latLng(c),roads:_locNearbySegments(_locSurfaceData.roads,c,.045),water:_locNearbySegments(_locSurfaceData.water,c,.045)};return _locSurfaceWindow;
}
function _ensureLocSurfaceData(c){
  if(_locSurfaceData){_buildLocSurfaceWindow(c);return Promise.resolve(_locSurfaceData);}
  if(_locSurfaceLoading)return _locSurfaceLoading;
  _locSurfaceLoading=Promise.all([
    fetch('roads.geojson?v='+DATAVER.roads).then(function(r){return r.ok?r.json():{features:[]};}),
    fetch('rivers.geojson?v='+DATAVER.rivers).then(function(r){return r.ok?r.json():{features:[]};})
  ]).then(function(rows){_locSurfaceData={roads:rows[0],water:rows[1]};_buildLocSurfaceWindow(c);return _locSurfaceData;}).catch(function(){_locSurfaceData={roads:{features:[]},water:{features:[]}};return _locSurfaceData;}).finally(function(){_locSurfaceLoading=null;});
  return _locSurfaceLoading;
}
function _locationSurfaceDistances(c){
  if(!_locSurfaceData)return {road:Infinity,water:Infinity};
  if(!_locSurfaceWindow||map.distance(c,_locSurfaceWindow.center)>2500)_buildLocSurfaceWindow(c);
  function nearest(rows){let d=Infinity;for(let i=0;i<rows.length;i++){d=Math.min(d,_locPointSegmentM(c,rows[i]));if(d<3)break;}return d;}
  return {road:nearest(_locSurfaceWindow.roads),water:nearest(_locSurfaceWindow.water)};
}
function _locModeFromEvidence(roadM,waterM,accuracy,speed){
  accuracy=Math.max(0,Number(accuracy)||0);speed=Number(speed);if(!isFinite(speed)||speed<0)speed=0;
  const roadLimit=Math.min(45,Math.max(16,accuracy*.7+10)),waterLimit=Math.min(160,Math.max(45,accuracy+25));
  if(speed>=6||roadM<=roadLimit)return 'car';
  if(waterM<=waterLimit)return 'canoe';
  return 'person';
}
function _classifyLocationMode(e){const d=_locationSurfaceDistances(e.latlng);return _locModeFromEvidence(d.road,d.water,e.accuracy,e.speed);}
function _setLocationMode(mode,force){
  if(mode!==_locMode&&!force){if(_locModePending===mode)_locModeHits++;else{_locModePending=mode;_locModeHits=1;}if(_locModeHits<2)return;}else{_locModePending='';_locModeHits=0;}
  if(mode===_locMode&&!force)return;_locMode=mode;_locModePending='';_locModeHits=0;if(_locMarker)_locMarker.setIcon(_locIcon(mode));_setLocateState(_locWatching,false);
}
const LocateCtl=L.Control.extend({ options:{position:'bottomleft'},
  onAdd:function(){ const d=L.DomUtil.create('div','locbtn'); d.id='locBtn'; d.title='내 위치';
    d.setAttribute('role','button'); d.setAttribute('aria-label','실시간 현재 위치 보기'); d.setAttribute('aria-pressed','false');
...
function _setLocateState(active,loading){
  const b=document.getElementById('locBtn'); if(!b)return;
  b.classList.toggle('active',!!active); b.classList.toggle('loading',!!loading);
  b.setAttribute('aria-pressed',active?'true':'false');
  b.title=active?'실시간 위치 추적 중 · '+_locModeLabel[_locMode]+' · 지도 이동·확대 중에도 유지':'내 위치';
}
function stopLocateFollow(){
  if(!_locWatching)return;
  _locWatching=false; _locHasFix=false; map.stopLocate(); _setLocateState(false,false);
}
function locateMe(){
  if(!navigator.geolocation){ L.popup().setLatLng(map.getCenter()).setContent('이 브라우저는 위치를 지원하지 않습니다').openOn(map); return; }
  if(_locWatching){
    if(_locMarker)map.setView(_locMarker.getLatLng(),map.getZoom(),{animate:true});
    return;
  }
  _locWatching=true; _locHasFix=false; _setLocateState(true,true);
...
map.on('locationfound', function(e){
  ...
  else _locCircle=L.circle(e.latlng,{radius:radius,color:'#1976d2',weight:1,fillColor:'#42a5f5',fillOpacity:.15}).addTo(map);
  if(_locMarker)_locMarker.setLatLng(e.latlng);
  else _locMarker=L.marker(e.latlng,{icon:_locIcon(_locMode),zIndexOffset:1000,interactive:false}).addTo(map);
  _ensureLocSurfaceData(e.latlng).then(function(){if(_locWatching&&_locLastEvent===e)_setLocationMode(_classifyLocationMode(e),true);});
  if(!_locHasFix){_locHasFix=true;map.setView(e.latlng,Math.max(map.getZoom(),16));}
  else map.panTo(e.latlng,{animate:true,duration:.35,noMoveStart:true});
});
...
const TRIP_QUEUE_KEY='mc_trip_queue_v1';
...
function _finishTripLocal(){tripBackupClear();if(_pendTrip)[_pendTrip.line,_pendTrip.planLine,_pendTrip.estimateLine,_pendTrip.posMarker].forEach(function(l){if(l)map.removeLayer(l);});_pendTrip=null;}
function _queueTrip(item){const q=_tripQueue();if(q.some(function(x){return x.clientId===item.clientId;}))return true;q.push(item);return _tripQueueWrite(q);}
async function flushOfflineTrips(){
  if(!navigator.onLine)return;const u=getUser();if(!u||!u.uid||!u.tok)return;let q=_tripQueue(),changed=false,sent=0;
  for(let i=0;i<q.length;){const item=q[i];if(String(item.uid)!==String(u.uid)){i++;continue;}try{await _sendTripItem(item,u);q.splice(i,1);changed=true;sent++;}catch(e){if(e&&e.status===401)toastMsg('대기 중인 투어 전송에 재로그인이 필요해요');break;}}
  if(changed)_tripQueueWrite(q);if(sent)toastMsg('✅ 오프라인 투어 '+sent+'건 전송 완료');
}
async function saveTrip(){ if(!_pendTrip) return; const u=getUser(); if(!u||!u.uid) return;
  ...
  const item=_tripQueuedItem(title,shared,saveAsCourse,u),msg=document.getElementById('tmMsg');msg.textContent=navigator.onLine?'저장 중…':'휴대폰에 저장 중…';
  try{if(!navigator.onLine)throw new Error('offline');await _sendTripItem(item,u);gaEvent('trip_save',{km:Math.round(_pendTrip.km*10)/10,shared:shared});_finishTripLocal();openTModal('trips');toastMsg(saveAsCourse?'투어 기록과 새 코스를 저장했어요':'투어 기록을 저장했어요');}
  catch(e){if(e&&e.status&&e.status<500){msg.textContent=e.status===401?'재로그인이 필요해요':'저장 실패';return;}if(_queueTrip(item)){_finishTripLocal();closeTModal();gaEvent('trip_queue');toastMsg('📴 휴대폰에 저장됨 · 연결 시 자동 전송');}else msg.textContent='기기 저장공간 부족 — 화면을 닫지 마세요';} }
...
function tripItem(x, mine){ const dt=x.start?new Date(x.start):null; const ds=dt?(dt.getMonth()+1)+'/'+dt.getDate():'';
  ...
  return h+'</div>'; }
async function renderTrips(){ const u=getUser(),queued=_tripQueue().filter(function(x){return String(x.uid)===String(u.uid);});let list=[];try{const r=await fetch(wapi('/trips?uid='+encodeURIComponent(u.uid)+'&tok='+encodeURIComponent(u.tok||'')));if(r.ok)list=await r.json();}catch(e){}
  const qh=queued.map(function(x){const s=x.summary||{};return '<div class="tm-item"><div class="ti-main"><div class="ti-t">📴 '+pmEsc(s.title||'투어')+'</div><div class="ti-s">전송 대기 · '+(Number(s.km)||0).toFixed(2)+'km · '+fmtDur(s.durSec||0)+'</div></div></div>';}).join('');
  document.getElementById('tmBody').innerHTML=tabBar('trips')+qh+(list.length?list.map(function(x){return tripItem(x,true);}).join(''):(queued.length?'':'<div class="tm-empty">아직 기록이 없어요. 하단 ▶ 카누잉 시작!</div>')); }
...
async function _offlineRemove(){const db=await _offlineDb();return new Promise(function(resolve,reject){const tx=db.transaction(OFFLINE_STORE,'readwrite');tx.objectStore(OFFLINE_STORE).delete(OFFLINE_KEY);tx.oncomplete=function(){resolve();db.close();};tx.onerror=function(){reject(tx.error);db.close();};});}
function _offInside(lat,lng,b){return isFinite(lat)&&isFinite(lng)&&lat>=b.s&&lat<=b.n&&lng>=b.w&&lng<=b.e;}
function _offBounds(course){
  let bb=null;if(course&&Array.isArray(course.coords)&&course.coords.length>1)try{bb=L.latLngBounds(course.coords);}catch(e){}
...
function _offCourse(){
  const focus=String(_courseFocusId||''),c=focus?(focus.charAt(0)==='k'?_kvCourses[focus.slice(1)]:_courseByCid[focus]):null;
  return (c&&c.coords&&c.coords.length>1)?c:((_offlineCandidate&&_offlineCandidate.coords&&_offlineCandidate.coords.length>1)?_offlineCandidate:null);
}
function _offCourseCopy(c){return c?{id:c.id,static:!!c.static,name:String(c.name||'코스').slice(0,100),km:Number(c.km)||0,color:validCourseColor(c.color),coords:c.coords.slice(0,10000).map(function(p){return[+p[0],+p[1]];})}:null;}
function _offDate(ms){const d=new Date(ms);return d.getFullYear()+'. '+(d.getMonth()+1)+'. '+d.getDate()+'. '+('0'+d.getHours()).slice(-2)+':'+('0'+d.getMinutes()).slice(-2);}
function _offProgress(n,text){const p=document.getElementById('offlineProgress'),i=p&&p.querySelector('i'),s=document.getElementById('offlineStatus');if(p)p.classList.toggle('on',n<100);if(i)i.style.width=Math.max(0,Math.min(100,n))+'%';if(s&&text)s.innerHTML='<b>'+pmEsc(text)+'</b><small>저장이 끝날 때까지 이 화면을 유지하세요.</small>';}
function _offlineStatus(){
  const s=document.getElementById('offlineStatus'),sv=document.getElementById('offlineSave'),us=document.getElementById('offlineUse'),del=document.getElementById('offlineDelete');if(!s)return;
...
function closeOfflineModal(){document.getElementById('offlineModal').classList.remove('open');}
function _offSanitizePlaces(b,u){if(!u||!u.uid)return[];return (_kvPlaces||[]).filter(function(x){return _offInside(+x.lat,+x.lng,b);}).slice(0,1000).map(function(x){const cat=x.cat==='명소'?'spot':(x.cat==='런칭/랜딩 후보지'?'candidate':'canoe');return{id:String(x.id||''),name:String(x.name||'').slice(0,80),memo:String(x.memo||'').slice(0,500),lat:+x.lat,lng:+x.lng,cat:cat,rv:!!x.rv};});}
...
function _restoreOfflinePrivate(pack){
  const u=getUser()||{};if(pack.uid&&String(pack.uid)===String(u.uid||''))(pack.places||[]).forEach(function(x){if(!_placeMarkerById[String(x.id)])_upsertSecurePlace(x);});
  (pack.obstacles||[]).forEach(function(x){if(!_obstacles[String(x.id)])renderObstacle(x);});
