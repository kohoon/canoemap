...
function _setLocateState(active,loading){
  ...
  b.title=active?('실시간 위치 추적 중 · '+_locModeLabel[_locMode]+' · '+(_locFollowView?'현재 위치 따라가기':'지도 자유 탐색 중')):'내 위치';
}
...
function pauseLocateViewFollow(){
  if(!_locWatching||_locInternalMove||!_locFollowView)return;
  _locFollowView=false;_setLocateState(true,false);
}
function _locSetInitialView(latlng){
  _locInternalMove=true;
  try{map.setView(latlng,Math.max(map.getZoom(),16));}finally{_locInternalMove=false;}
}
function locateMe(){
  ...
  _locWatching=true;_locHasFix=false;_locFollowView=true;_setLocateState(true,true);
