...
let _daisoFeatures=null,_daisoMeta={},_daisoLoading=null,_daisoAttributed=false;
...
function _loadDaiso(){
  ...
  _daisoLoading=fetch('./daiso_stores.geojson?v='+DATAVER.daiso).then(function(r){if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(function(fc){_daisoFeatures=fc.features||[];_daisoMeta=fc.metadata||{};if(!_daisoAttributed){map.attributionControl.addAttribution('다이소 지점 &copy; ㈜아성다이소 공식 매장검색');_daisoAttributed=true;}return _daisoFeatures;}).finally(function(){_daisoLoading=null;});return _daisoLoading;
}
...
function _setLocateState(active,loading){
  ...
  b.title=active?'실시간 위치 추적 중 · '+_locModeLabel[_locMode]+' · 지도를 끌면 종료':'내 위치';
}
...
map.on('dragstart',stopLocateFollow);
