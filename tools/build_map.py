...
function _shortCoursePlace(label,addr){
  label=(label||'').trim();
  function regionText(value){const p=(value||'').trim().split(/\s+/).filter(Boolean);let i=p.findIndex(function(x){return /(특별시|광역시|특별자치시|시|군|구)$/.test(x)&&!/(특별자치도|도)$/.test(x);});if(i<0)return p.filter(function(x){return !/(특별자치도|도)$/.test(x);}).join(' ');const r=[p[i]];if(/시$/.test(p[i])&&p[i+1]&&/구$/.test(p[i+1]))r.push(p[i+1]);return r.join(' ');}
  if(label){const halves=label.split(/\s+-\s+/),raw=(halves[0]||'').replace(/\([^)]*\)/g,'').trim(),region=regionText(raw),place=(halves.length>1?halves.slice(1).join(' - '):label).replace(/^(춘천|양구|곡성|구례|연천|홍천|인제|화천)\s*[()]/,'').trim();return (region&&place.indexOf(region)!==0?region+' ':'')+place;}
  const parts=(addr||'').trim().split(/\s+/).filter(Boolean),region=regionText(addr);let place='';
  for(const p of parts){if(/[리가동]$/.test(p))place=p;else if(!place&&/[읍면]$/.test(p))place=p;}
  return [region,place].filter(Boolean).join(' ')||parts.filter(function(x){return !/(특별자치도|도)$/.test(x);}).slice(-2).join(' ');
}
