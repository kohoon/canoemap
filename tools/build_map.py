...
function _shortCoursePlace(label,addr){
  label=(label||'').trim();
  function localAddress(value){const p=(value||'').trim().split(/\s+/).filter(Boolean);let i=p.findIndex(function(x){return /(특별시|광역시|특별자치시|시|군|구)$/.test(x)&&!/(특별자치도|도)$/.test(x);});return (i>=0?p.slice(i):p.filter(function(x){return !/(특별자치도|도)$/.test(x);})).join(' ');}
  ...
  if(label){const halves=label.split(/\s+-\s+/),raw=(halves[0]||'').replace(/\([^)]*\)/g,'').trim(),region=regionText(raw),place=(halves.length>1?halves.slice(1).join(' - '):label).replace(/^(춘천|양구|곡성|구례|연천|홍천|인제|화천)\s*[()]/,'').trim();if(halves.length>1&&/(?:특별자치도|도|특별시|광역시|특별자치시|\S+[시군구])\s/.test(place))return localAddress(place);return (region&&place.indexOf(region)!==0?region+' ':'')+place;}
