...
async function showAddress(lat,lng){
  ...
  const pop=L.popup({offset:L.point(0,-52),keepInView:true,autoPanPadding:[18,18]}).setLatLng([lat,lng]).setContent('주소 조회 중…').openOn(map);
...
map.on('popupclose',clearLandOwnership);
// ---- 어드민: 장소 등록 ----
...
async function waterRoute(p1,p2){
  ...
  // 춘천호 오월1교 서쪽 수면은 도로·산림 둑으로 본류와 분리되어 카누로 통과할 수 없다.
  ...
  for(const el of (j.elements||[])){
    ...
    for(const p of el.geometry){ const k=key(p.lat,p.lon); if(wayName)(nodeNames[k]||(nodeNames[k]={}))[wayName]=((nodeNames[k]||{})[wayName]||0)+1;if(prev&&prev!==k&&!crossesNavigationBarrier(nodes[prev],nodes[k])){ const d=hav(nodes[prev],nodes[k]);
