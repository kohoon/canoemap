...
function _obDisplayName(o){const t=OBS_TYPES[o&&o.type]||OBS_TYPES['보'];if(o&&o.type==='캠핑사이트')return t.label;return (o&&o.name&&String(o.name).trim())?String(o.name).trim():t.label;}
function obsIcon(type,name){const t=OBS_TYPES[type]||OBS_TYPES['보'],showName=type==='유명지',raw=(name&&String(name).trim())?String(name).trim():t.label,disp=pmEsc(raw);return L.divIcon({className:'obs-div',html:'<span class="obs-ic '+t.c+(showName?'':' obs-icon-only')+'" aria-label="'+pmEsc(showName?raw:t.label)+'">'+t.e+(showName?' '+disp:'')+'</span>',iconSize:null});}
